
long int byte_timout_rtu[5]={20,20,20,20,20};

int ffgets_com_rtu( int ii)
{
//  Адрес функции будет устанавлен в указателе  ff_gets_com[Port]
// при посылке команды в устройство Modbus RTU.
//  Функция принимает символы из порта COM ii  в буфер cb_COM[ii].
//  При таймауте команда считается введенной и функция возвращает
// значение 1.
//

/* ii - номер COM   1...4 */
 char tmp;
m1:
     if(IsCom(ii)==0)
     {
         if( f_timer(time_snd[ii],ToutAns[ii]))
         { // истек таймаут приема ответа на команду
            return 1;
         }
         return 0;
     }
     tmp=ReadCom(ii);
     ToutAns[ii]=byte_timout_rtu[ii];

     cb_COM[ii][n_bcom[ii]]=tmp;
     if( (++n_bcom[ii]) >= n_bufcom )
      {
        n_bcom[ii]=0;
  /*
        printf("\n Command buffer overflow !\n--");
        puts(cb_COM1);
  */
      }
   time_snd[ii]= TimeStamp;
   goto m1;
}
//-----------------------------
#define  tmp_lgth 60

int f_chg_toRTU(char *buf)
{  // преобразует строку текста в сообщение RTU
int nn,ii,n2,i;
char buf_tmp[tmp_lgth];
char *buf_c;
   buf_c=buf;
   nn=strlen(buf);
   if(nn & 0x1) return -1; // нечетное количество символов - возможна ошибка
   n2=nn>>1;
   if( n2 >= tmp_lgth) return -2; // нет места в буфере
   for(i=0;i<n2;i++)
   {
    if(sscanf(buf_c,"%2x",&buf_tmp[i]) <= 0) return -3; // ошибка при вводе
    buf_c+=2;
   }
   _fmemcpy(buf,buf_tmp,(long int)n2);
   return n2;
}
//-----------------------------
/* Fast CRC */
unsigned int  crc16(unsigned char *buffer, int buffer_length)
{
unsigned  char crc_hi = 0xFF; /* high CRC byte initialized */
unsigned  char crc_lo = 0xFF; /* low CRC byte initialized */
unsigned  int i; /* will index into CRC lookup */
  unsigned int summ;

        /* pass through message buffer */
        while (buffer_length--) {
                i = crc_hi ^ *buffer++; /* calculate the CRC  */
                crc_hi = crc_lo ^ table_crc_hi[i];
                crc_lo = table_crc_lo[i];
        }
  summ = ((((unsigned int)crc_hi << 8) & 0xff00) | ((unsigned int) crc_lo &0xff));
        return (summ);
}

//-----------------------------
int f_check_crc_RTU( char *msg,int msg_length)
{
 // функция проверяет команду в буфере *buf на корректность контрольной суммы
 // Возвращает  1 -OK
 //             0 - error CRC

        int ret;
        unsigned int crc_calc;
        unsigned int crc_received;

        crc_calc = crc16(msg, msg_length - 2);
        crc_received = ((unsigned int)msg[msg_length - 2] << 8) | ( msg[msg_length - 1] & 0xff);

        /* Check CRC of msg */
        if (crc_calc == crc_received)  ret = 1;
        else  ret=0;
        return ret;
}
/*----------------------------*/
int f_prepareRTU(char *msg,int *msg_length)
{ //  функция вычисляет конторольную сумму команды в буфере msg
  //  и записывает  ее после команды.
  //  Длина сообщения увеличивается на 2 байта
  //  Возвращает 0 - OK,
  //             1 - нет места в буфере

unsigned int crc_calc;
int i,i1;

  i1=*msg_length;
  if( i1 >= (n_buf_queue-3)) return 1;
  crc_calc = crc16(msg,i1);
//QQQQQ
  msg[i1++]=(crc_calc >> 8) & 0xff;
  msg[i1++]=crc_calc & 0xff;
  *msg_length +=2;

  return 0;

}
/*----------------------------*/
int f_prepareRTU1(char *msg,int *msg_length)
{ //  функция вычисляет конторольную сумму команды в буфере msg
  //  и записывает  ее после команды.
  //  Длина сообщения увеличивается на 2 байта
  //  Возвращает 0 - OK,
  //             1 - нет места в буфере

unsigned int crc_calc;
int i,i1;

  i1=*msg_length;
  if( i1 >= (n_buf_RTU_s-3)) return 1;
  crc_calc = crc16(msg,i1);
//QQQQQ
  msg[i1++]=(crc_calc >> 8) & 0xff;
  msg[i1++]=crc_calc & 0xff;
  *msg_length +=2;

  return 0;

}
/*----------------------------*/

unsigned char table_crc_hi[] = {
        
};

/* Table of CRC values for low-order byte */
unsigned char table_crc_lo[] = {
        
};


